...
from .attention_constrain_loss import AttentionConstrainedLoss
...
class AnchorHeadMultiAtten(AnchorHeadTemplate):
    ...
    def forward(self, data_dict):
        ...
        # ### get attention via cbam of fused features
        ...
        ret = {
            ...
            'atten_maps': atten_maps,
        } ### add attention maps into the ret_dict
        ...
        if self.training:
            ...
            #### add target dict into the dict
            self.forward_ret_dict.update({'gt_boxes': data_dict['gt_boxes']})
    ...
    def get_attention_constrain_loss(self):
        atten_losses = 0.0
        # self.atten_loss = []
        # self.tasks = [
        ...
        #         ]
        # for _, target in enumerate(self.tasks):
            # self.atten_loss.append(
            #         AttentionConstrainedLoss(,
            #             [-51.2, -51.2, -5.0, 51.2, 51.2, 3.0], self.num_class, self.atten_res)# task_id
                # )
        self.atten_loss = AttentionConstrainedLoss([-51.2, -51.2, -5.0, 51.2, 51.2, 3.0], self.num_class, self.atten_res)
                
        batch_atten_map = self.forward_ret_dict['atten_maps']

        for task_id in range(len(batch_atten_map)):  ## go through the batch
            batch_boxes = self.forward_ret_dict["gt_boxes"] ## ground-truth boxes
            #batch_task_boxes = [batch_box[task_id] for batch_box in batch_boxes]

            batch_labels = self.forward_ret_dict['box_cls_labels']  ## ground-truth labels
            ...
            for index, bam in enumerate(batch_atten_map):
                temp_attention_loss = self.atten_loss(
                    bam, batch_boxes, batch_labels)
            ...
            for value in attention_loss.values():
                ...
                atten_losses += value
        tb_dict = {
            'rpn_atten_loss': atten_losses.item()
        }
        return atten_losses, tb_dict

    def get_loss(self):
        cls_loss, tb_dict = self.get_cls_layer_loss()
        box_loss, tb_dict_box = self.get_box_reg_layer_loss()
        atten_loss, tb_dict_att = self.get_attention_constrain_loss()
        tb_dict.update(tb_dict_box)
        rpn_loss = cls_loss + box_loss + atten_loss
