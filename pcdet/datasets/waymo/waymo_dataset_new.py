...
class WaymoDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        self.infos = []
        if self.dataset_cfg.MULTI_SWEEP:
            self.pose_list = []
            self.sweep_list = []
    ...
    def include_waymo_data(self, mode):
        ...
        if self.dataset_cfg.SAMPLED_INTERVAL[mode] > 1:
            ...
            for k in range(0, len(self.infos), self.dataset_cfg.SAMPLED_INTERVAL[mode]):
                ...
                # info = copy.deepcopy(self.infos[k])
                if self.dataset_cfg.MULTI_SWEEP:
                    pose_list, sweep_list = self.include_waymo_pose(self.infos[k]['point_cloud']['sample_idx'], k)
                    self.pose_list.append(pose_list)
                    self.sweep_list.append(sweep_list)
            self.infos = sampled_waymo_infos
            self.logger.info('Total sampled samples for Waymo dataset: %d' % len(self.infos))
        else:
            for k in range(0, len(self.infos)):
                # info = copy.deepcopy(self.infos[k])
                if self.dataset_cfg.MULTI_SWEEP:
                    pose_list, sweep_list = self.include_waymo_pose(self.infos[k]['point_cloud']['sample_idx'], k)
                    self.pose_list.append(pose_list)
    ...
    def include_waymo_pose(self, sample_idx, data_idx):
        ## get the sample_idx for multi sweeps
        pose_list = []
        num_sweeps_pre = self.dataset_cfg.NUM_SWEEP - 1
        if sample_idx < self.dataset_cfg.NUM_SWEEP - 1:
            sweep_idx_pre2 = list(range(sample_idx))
            sweep_idx_pre1 = [0 for t in range(num_sweeps_pre - len(sweep_idx_pre2))]
            sweep_pre_idx = sweep_idx_pre1 + sweep_idx_pre2 # sweep_pre_idx: this is the previous sweep sample_idx
        else:
            sweep_pre_idx = list(range(sample_idx-num_sweeps_pre, sample_idx))

        assert len(sweep_pre_idx) == num_sweeps_pre, 'Error in previous sweeps!'

        ### put pose matrices into a list
        for k in sweep_pre_idx:
            i = data_idx - (sample_idx-k)
            # info = copy.deepcopy(self.infos[i])
            pose_list.append(self.infos[i]['pose'])
        # info = copy.deepcopy(self.infos[data_idx])
        pose_list.append(self.infos[data_idx]['pose'])
        sweep_pre_idx.append(sample_idx)
        return pose_list, sweep_pre_idx
    ...
    def get_lidar_multi(self, sequence_name, sample_idx_list, pose_list):
        sweeps_point = []
        sweeps_time = []
        ## load points for current frame
        curr_sample_idx = sample_idx_list[-1]

        ## load points for previous frames
        for t in range(len(sample_idx_list)):
            pre_points_all = []
            pre_points_all = self.get_lidar(sequence_name, sample_idx_list[t])
            pre_points_all = self.transform_point_cloud(pre_points_all, pose_list[t], pose_list[-1])
            pre_time = sample_idx_list[t] * np.ones((pre_points_all.shape[0], 1)) - curr_sample_idx
            sweeps_time.append(pre_time)
            sweeps_point.append(pre_points_all)
      
        sweeps_point = np.concatenate(sweeps_point, axis=0)
        sweeps_time = np.concatenate(sweeps_time, axis=0).astype(sweeps_point.dtype)
        points = np.concatenate((sweeps_point, sweeps_time), axis=1)  # points dim = [N,6]
        return points

    def transform_point_cloud(self, pc, old_pose, new_pose):
        """Transform a point cloud from one vehicle frame to another.
        Args:
            pc: N x 5 float32 point cloud, where the first three dimensions are the
    ...
    def __getitem__(self, index):
        ...
        if self.use_shared_memory and index < self.shared_memory_file_limit:
            ...
            if self.dataset_cfg.MULTI_SWEEP:
                points = self.get_lidar_multi(sequence_name, self.sweep_list[index], self.pose_list[index])
            else:
                points = self.get_lidar(sequence_name, sample_idx)
